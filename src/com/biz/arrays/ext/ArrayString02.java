...
public class ArrayString02 {
	...
	public static void main(String[] args) {
		...
		// strNation에 저장되어있는 문자열을 빈칸을 기준으로 나누어서 한개씩 분할하여 각각의 배열에 담음
		for(int i = 0 ; i < arrWord.length ; i ++) {
			System.out.println(arrWord[i]);
		}
		// arrWord[0] = Republic, arrWord[1] = of, arrWord[2] = Korea
		
		strNation = "대 한 민 국 만세";
		arrWord = strNation.split(" ");
		
		System.out.println(arrWord.length);			//5개
		for(int i = 0 ; i < arrWord.length ; i ++) {
			System.out.println(arrWord[i]);
		}	// arrWord[0] = 대, arrWord[1] = 한, arrWord[2] = 민, arrWord[3] = 국, arrWord[4] = 만세  
		...
		int intMth = Integer.valueOf(arrGrade[4]);
